...
public class WindowWithdraw extends JFrame {
  ...
  public WindowWithdraw(String accountName,BankAccount chequing,BankAccount savings) {
    ...
    setTitle("Withdrawing"); //setting up window
    ...
    btnCalculate.addMouseListener(new MouseAdapter() { // when button is pressed to calculate the balance 
    ...
    cancel.addMouseListener(new MouseAdapter() { //returning user back to windowOne if button is clicked  
      public void mouseClicked(MouseEvent e) {
        returnBack();
      }
    });
  ...
  public void calculate() {//updating balance
    double currentBalance= Double.valueOf(textFieldOne.getText()); 
    if(accountName.equals("Chequing")){ 
      chequing.withdraw(currentBalance,"Withdrawal"); //description for transactions
    } else {
